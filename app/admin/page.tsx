...
export default function AdminPage() {
  const { status, token } = useAuthGuard(['admin']);
  ...
  useEffect(() => {
    ...
    async function loadStats(authToken: string) {
      const response = await fetch('/api/admin/stats', { headers: { Authorization: `Bearer ${authToken}` } });
      if (response.ok) {
  ...
  if (status === 'denied') {
    return <GuardScreen title="Access denied" message="You must be signed in as an administrator to view this dashboard." />;
  }
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <button onClick={() => downloadReport('/api/reports/pdf', 'election-summary.pdf')} className="inline-flex rounded-md bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700">Download PDF Summary</button>
            <button onClick={() => downloadReport('/api/reports/excel', 'turnout.xlsx')} className="inline-flex rounded-md border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-900 hover:bg-slate-50">Download Excel Turnout</button>
          </div>
        </Card>
        <Card>
